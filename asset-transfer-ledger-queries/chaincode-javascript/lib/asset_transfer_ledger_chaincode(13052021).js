...
// Index for docType, owner, creator (descending order).
//
// Example curl command line to define index in the CouchDB channel_chaincode database
// curl -i -X POST -H "Content-Type: application/json" -d "{\"index\":{\"fields\":[{\"creator\":\"desc\"},{\"docType\":\"desc\"},{\"owner\":\"desc\"}]},\"ddoc\":\"indexSizeSortDoc\", \"name\":\"indexSizeSortDesc\",\"type\":\"json\"}" http://hostname:port/myc1_assets/_index
...
//   peer chaincode query -C CHANNEL_NAME -n asset_transfer -c '{"Args":["QueryAssets","{\"selector\":{\"docType\":{\"$eq\":\"asset\"},\"owner\":{\"$eq\":\"Tom\"},\"creator\":{\"$gt\":0}},\"fields\":[\"docType\",\"owner\",\"creator\"],\"sort\":[{\"creator\":\"desc\"}],\"use_index\":\"_design/indexSizeSortDoc\"}"]}'
...
class Chaincode extends Contract {
	...
	async DeleteAsset(ctx, id) {
		...
		// to maintain the category~name index, we need to read the asset first and get its category
		...
		// delete the index
		let indexName = 'category~name';
		let categoryNameIndexKey = ctx.stub.createCompositeKey(indexName, [assetJSON.category, assetJSON.docID]);
		if (!categoryNameIndexKey) {
			throw new Error(' Failed to create the createCompositeKey');
		}
		//  Delete index entry to state.
		await ctx.stub.deleteState(categoryNameIndexKey);
	}
	...
	async TransferAssetByCategory(ctx, category, newOwner) {
		...
		let responseRange = await categoryedAssetResultsIterator.next();
		while (!responseRange.done) {
			...
			responseRange = await categoryedAssetResultsIterator.next();
		}
	}
	...
	// page creator and a bookmark.
	// The number of fetched records will be equal to or lesser than the page creator.
	...
	// QueryAssetsWithPagination uses a query string, page creator and a bookmark to perform a query
	// for assets. Query string matching state database syntax is passed in and executed as is.
	// The number of fetched records would be equal to or lesser than the specified page creator.
	// Supports ad hoc queries that can be defined at runtime by the client.
	// If this is not desired, follow the QueryAssetsForOwner example for parameterized queries.
	// Only available on state databases that support rich query (e.g. CouchDB)
